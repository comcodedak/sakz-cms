...
(
   ...
   f_type               int not null comment '类型(文字[1]-图片[2]-FLASH[3]-代码[4])',
...
(
   ...
   f_status             int not null default 0 comment '状态(就绪[0]-运行中[1]-暂停[2])',
...
(
   ...
   f_type               int not null default 1 comment '类型(系统字段[1]-custom字段[2]-clob字段[3])',
   f_source_type        int not null default 1 comment '来源(详细页[1]-列表页[2]-固定值[3]-URL[4])',
...
(
   ...
   f_time               datetime not null comment '时间',
   f_status             int not null default 0 comment '状态(成功[0]-失败[1])',
...
(
   ...
   f_status             int not null default 0 comment '未审核[0]-已审核[1]-推荐[2]-屏蔽[3]',
   f_text               mediumtext,
...
(
   ...
   f_status             int not null default 0 comment '状态(已审核[0]-未审核[1])',
...
(
   ...
   f_is_recommend       char(1) not null default '0' comment '是否推荐',
   f_status             int not null default 0 comment '状态(已审核[0]-未审核[1]-屏蔽[2])',
...
(
   ...
   f_status             char(1) not null default 'A' comment '状态(发起者[0]-审核中[1-9]-已发布[A]-草稿[B]-投稿[C]-退稿[D]-采集[E]-待发布[F]-已过期[G]-回收站[X]-归档[Z])',
   ...
   f_html_status        char(1) not null default '0' comment '静态HTML状态(未开启[0]-待生成[1]-待更新[2]-已生成[3])',
...
(
   ...
   f_type               int not null default 0 comment '类型(普通组[0]-游客组[1]-IP组[2]-待验证组[3])',
   f_seq                int not null default 2147483647 comment '排序',
...
(
   ...
   send_time_           datetime not null comment '发送时间',
   deletion_flag_       int not null default 0 comment '删除标志(正常[0]-发件人删除[1]-收件人删除[2])',
   is_unread_           int not null default 1 comment '是否未读',
...
(
   ...
   f_site_id            int not null comment '站点',
   f_type               varchar(100) not null comment '类型(文档[info]-栏目[node]-首页[node_home]-专题[special])',
   f_name               varchar(50) not null comment '名称',
...
(
   ...
   f_inner_type         int not null default 0 comment '内部类型(用户自定义字段[0]-系统定义字段[1]-预留大文本字段[2]预留可查询字段[3])',
...
(
   ...
   f_static_method      int comment '静态页生成方式(手动生成[0]-自动生成栏目页[1]-自动生成文档页及栏目页[2]-自动生成文档页、栏目页、父栏目页、首页[3])',
...
(
   ...
   f_area               varchar(100) comment '地区',
   f_time               datetime not null comment '时间',
   f_type               int not null default 1 comment '类型(操作日志[1]-登录日志[2]-登录失败[3])',
...
(
   ...
   f_seq                int not null default 2147483647 comment '排列顺序',
   f_method             int not null default 1 comment '方式(文件系统[1]-FTP[2])',
   f_type               int not null default 1 comment '类型(HTML发布[1]-附件发布[2])',
...
(
   ...
   f_mode               int not null default 1 comment '模式(独立访客[1]-独立IP[2]-独立用户[3])',
   f_interval           int not null default 0 comment '间隔时间（天）',
   f_total              int not null default 0 comment '总票数',
   f_status             int not null default 0 comment '状态(启用[0]-禁用[1])',
...
(
   ...
   f_info_perm_type     int not null default 1 comment '文档权限类型(所有[1]-组织[2]-自身[3])',
...
(
   ...
   f_unit               int comment '时间单位(毫秒[1]-秒[2]-分[3]-时[4]-天[5]-周[6]-月[7]-年[8])',
   f_cycle              int not null default 1 comment '执行周期(cron[1]-simple[2])',
   f_status             int not null default 0 comment '状态(启用[0]-禁用[1])',
...
(
   ...
   f_status             int not null default 0 comment '状态(正常[0]-禁用[1])',
   f_tree_number        varchar(100) not null default '0000' comment '树编码',
   f_tree_level         int not null default 0 comment '树级别',
   f_tree_max           varchar(10) not null default '0000' comment '树子节点最大编码',
...
(
   ...
   f_type               int not null default 1 comment '类型(栏目HTML生成[1]-文档HTML生成[2]-全文索引生成[3])',
   f_status             int not null default 0 comment '状态(运行中[0]-完成[1]-中止[2]-停止[3])',
...
(
   ...
   f_type               int not null default 0 comment '类型(会员[0]-管理员[1])',
   f_status             int not null default 0 comment '状态(正常[0]-锁定[1]-待验证[2])',
...
(
   ...
   f_description        varchar(255) comment '描述',
   f_seq                int not null default 2147483647 comment '排序',
   f_status             int not null default 1 comment '状态(启用[1]-禁用[2])',
...
(
   ...
   f_creation_date      datetime not null comment '记录创建时间',
   f_opinion            varchar(255) comment '意见',
   f_type               int not null comment '类型(前进[1]-后退[2]-原地[3])',
...
(
   ...
   f_data_type          int not null comment '数据类型(文档[1])',
